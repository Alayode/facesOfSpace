...
juice.get('/api/characters', function (req, res, next) {
    ...
    Character.find({ random: { $near: [Math.random(), 0] } }).where('voted', false).where('gender', randomGender).limit(2).exec(function (err, characters) {
        ...
        Character.find({ random: { $near: [Math.random(), 0] } }).where('voted', false).where('gender', oppositeGender).limit(2).exec(function (err, characters) {
            if (err) return next(err);
...
juice.get('/api/characters/top', function (req, res, next) {
    ...
    .limit(50).exec(function (err, characters) {
        if (err) return next(err);
